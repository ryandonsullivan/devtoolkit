...
/**
 * Main Class file for `SWG_Toolkit_Settings`
 *
...
if ( ! class_exists( 'SWG_Toolkit_Settings' ) ) :
	...
	class SWG_Toolkit_Settings {
		...
		function callback_checkbox( $args ) {
			...
			$html  = '<fieldset>';
			$html .= sprintf( '<label for="swg-toolkit-%1$s[%2$s]">', $args['section'], $args['id'] );
			$html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
			$html .= sprintf( '<input type="checkbox" class="checkbox" id="swg-toolkit-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $args['section'], $args['id'], checked( $value, 'on', false ) );
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				...
				$html   .= sprintf( '<label for="swg-toolkit-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html   .= sprintf( '<input type="checkbox" class="checkbox" id="swg-toolkit-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $checked, $key, false ) );
		...
		function callback_radio( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				$html .= sprintf( '<label for="swg-toolkit-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="radio" class="radio" id="swg-toolkit-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
				$html .= sprintf( '%1$s</label><br>', $label );
			}
			$html .= $this->get_field_description( $args );
			$html .= '</fieldset>';
		...
		public function admin_menu() {
			...
			add_options_page(
				...
				'SWG Toolkit',
				'manage_options',
				'swg_toolkit_admin_settings',
				array( $this, 'plugin_page' )
			);
		}

		public function plugin_page() {
			echo '<div class="wrap swg-toolkit">';
			echo '<h1>SWG Toolkit Settings <span style="font-size:50%;">v' . SWG_TOOLKIT_ADMIN_VERSION . '</span></h1>';
